...
- **Today** (`/today`) — a focused, single-day view for confirming what actually happened, day by day, as the week runs (rather than scanning the whole roster grid). Defaults to today with prev/next day navigation and a "Jump to today" shortcut once you've stepped away; every job scheduled that day gets a card with a quick "Mark complete" / "Cancel" action if still outstanding, or a clear resolved badge (+ hours) and an "Edit" affordance if already settled. A day-level "X of Y jobs confirmed" summary sits at the top, with an all-done empty state once nothing's left outstanding. Reuses the exact same `jobs.complete`/`jobs.cancel` mutations and `CompleteJobModal`/`CancelJobModal` as the roster — this is a filtered view over the same jobs, not a separate system.
