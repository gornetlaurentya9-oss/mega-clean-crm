...
`clients.remove`/`employees.remove` default to the same guardrail as `jobs.remove`: blocked once the record has any job history, since deleting it would either cascade-destroy that history (clients — `jobs.clientId` cascades on delete) or orphan it (employees — `jobs.employeeId` isn't a real FK, so old jobs would silently point at a deleted employee).
...
- **Employee force-delete**: any jobs referencing the employee get **unassigned** (`employeeId` set to `null`), not deleted — an employee's own record disappearing shouldn't also erase the client-side job/invoice history of work that was actually done. Time-off records and any `defaultEmployeeId` references (on clients or recurring patterns) are cleared the same as a normal delete.
