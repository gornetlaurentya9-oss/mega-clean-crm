...
- **Clients** (`/clients`) — searchable/filterable list, fast-entry add form (only name + phone required), detail page with prominent access notes and inline recurring-pattern management. A client can be permanently deleted from their detail page, but only if they have no job history yet (jobs.clientId cascades on delete, so a client with real history would silently take their invoicing/roster records with them) — otherwise deletion is blocked with a message pointing at Deactivate instead, which hides them from active use while keeping history intact.
- **Employees** (`/employees`) — list, add/edit, active/inactive toggle, time-off date ranges. Same permanent-delete rule as Clients: blocked once an employee has any job history (their `jobs.employeeId` reference isn't cascading, so deleting them would otherwise leave old jobs pointing at a deleted employee), with Mark inactive as the history-preserving alternative.
...
- Same "permanent delete, but only when there's no history to lose" principle as deleting a Client or Employee (see the Clients/Employees feature-tour entries above) — consistent guardrail across the app rather than a one-off rule just for jobs.
